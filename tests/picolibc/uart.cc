...
inline void uart_write_reg(uint8_t reg, uint8_t val)
{
    *(UART_REG(reg))= val;
}

inline uint8_t uart_read_reg(uint8_t reg)
{
    return *(UART_REG(reg));
}
...
void uart_init()
{
     ...
     */
    lcr= 0;
    uart_write_reg(LCR, lcr | (3 << 0)); // 8-bit word length
}
...
int uart_putc(char ch)
{
    while((uart_read_reg(LSR) & LSR_TX_IDLE) == 0)
        ;
    uart_write_reg(THR, static_cast<uint8_t>(ch));
    return static_cast<uint8_t>(ch);
}

void uart_puts(const char *pS)
{
    while(*pS != '\0') {
        uart_putc(*pS++);
    }
...
void uart_put_int(long num)
{
    if(num < 0) {
        uart_putc('-');
        num= -num;
    }
    uart_put_uint(static_cast<uint32_t>(num));
}

void uart_put_int(int32_t num)
{
    if(num < 0) {
        uart_putc('-');
        num= -num;
...
void uart_put_hex(uint32_t num)
{
    for(int i= 28; i >= 0; i-= 4) {
        uart_putc(G_HEX[(num >> i) & 0xF]);
    }
}

void uart_put_long_hex(uint64_t num)
{
