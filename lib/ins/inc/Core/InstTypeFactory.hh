...
public:
    static std::unique_ptr<IBaseInstType> CreateType(uint16_t opcode);
    static std::unique_ptr<IBaseInstType> CreateType(uint32_t inst);

private:
    const inline static std::unordered_map<uint16_t, InstFormat> G_Opcode2Format= {
